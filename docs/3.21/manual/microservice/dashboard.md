...
* 统一平台上的所有图表。<br>
* 可供用户灵活的自定义数据视图。<br>
* 可供用户展示自定义的指标数据和日志指标数据。<br>
* 解决沟通准确性与时间成本问题，把选择权交给用户。
  
## 前言
有一定SQL基础可以帮你更快上手。

## 引导
运维大盘是大盘系统的一部分，可以快速搭配出大盘以监控各种资源的实时情况，使用方法与其它大盘模块大致相同。
...
1、特别复杂的 AND OR 关系过滤条件组合，可以使用表达式，比如：<br>(tag1='value1' OR tag2='value2') AND (field1>=0 OR field2<100)。
