...
若采用平台提供的运行容器，平台将自动关闭 `RollingFileAppender` 等非 `ConsoleAppender` 的 appender，以确保日志无法写入磁盘。请确认已配置 `ConsoleAppender`，否则将无法查看日志。
...
此外，如需使用本地 IDE 进行连接，请进入 **DevOps 平台 > 我的应用 > 选择应用 > 部署中心 > 部署总览 > 容器列表** 找到需调试的容器 IP，在 IDE 新建 Remote 配置如下：
