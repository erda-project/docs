...
假设有这样的场景：某个应用在执行某段逻辑是抛出了异常，通过关键词查询您在日志列表中找到了这条关键的异常堆栈日志，但关于这段逻辑的请求上下文参数信息是通过再之前一点位置的另一条日志打印的，这时候您可能想查看当前这条异常日志之前一点位置的日志，而 `上下文浏览` 功能便是为此类场景而设计。

如下图，日志列表中每条日志旁都有 上下文浏览的入口，点击弹出 上下文浏览的弹窗页面。
...
上下文浏览页面，页面顶部默认会展示 `application_name`、`service_name`、`pod_name` 三个过滤标签，表示默认是浏览同一个 k8s pod 层面某条关键日志前后的日志，列表中标号为 `0` 的为刚才点击的关键日志。在该页面您还可以进行自定义显示的字段、自定义高亮内容等操作。

## 实时Tail

实时Tail的功能类似 `tail -f ｜ grep` 指令，可以查看某条日志之后是否有再出现符合筛选条件的日志。

如下图，日志列表中每条日志旁都有 实时Tail的入口，点击弹出 实时Tail的弹窗页面。
...
实时Tail 页面的输入条件比较灵活，允许输入查询语句，编辑完查询条件后可以重新点击页面右侧的开始按钮自动刷新加载符合条件的日志。
