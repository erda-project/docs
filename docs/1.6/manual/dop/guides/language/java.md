...
进入 **我的应用 > 选择应用 > 应用设置 > 流水线 > 变量配置 > 选择环境**。
...
为开启远程调试，需进入 **我的应用 > 选择应用 > 应用设置 > 流水线 > 变量配置**，添加 JAVA_OPTS 变量配置如下：
...
此外，如需使用本地 IDE 进行连接，请进入 **我的应用 > 选择应用 > 部署中心 > 部署总览 > 容器列表** 中找到需调试的容器 IP，在 IDE 新建 Remote 配置如下：
